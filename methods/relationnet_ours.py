...
class RelationNet_Ours(MetaTemplate):
    ...
    def set_forward(self,x,is_feature = False):
        ...
        z_support     = z_support.view( self.n_way, self.n_support, *self.feat_dim )#.mean(1) 

        assert self.n_support > 1, 'Other conditions are not considered' 


        z_proto = self.adaptive_module(z_support.transpose(1, 2).contiguous().view(-1, self.n_support, *self.feat_size)).view(self.n_way, self.dim, *self.feat_size)
